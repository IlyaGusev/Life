...
bool CMapWindow::Create(wchar_t* name, HINSTANCE hInstance, HWND hnd, int x, int y, int w, int h){
	handle = CreateWindowEx(0, L"MapWindow", name,
		WS_CHILD | WS_BORDER, x, y, w, h, hnd, 0, hInstance, this);
	this->Show();
	return true;
}
...
LRESULT __stdcall CMapWindow::windowProc(HWND hnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	CMapWindow* that = reinterpret_cast<CMapWindow*>(::GetWindowLong(hnd, GWL_USERDATA));
	switch(message){
		case WM_CREATE:
			that->handle = hnd;
			that->OnCreate();
